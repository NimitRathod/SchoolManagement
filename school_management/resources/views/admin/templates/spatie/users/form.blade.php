            ...
            <div class="row">
                ...
                <div class="col-md-4 pb-2">
                    <div class="form-group">
                        ...
                        <select class="form-control select2 w-100 @error('role') is-invalid @enderror" name="role" required >
                            <option disabled selected>Select role</option>
                            @foreach($roles AS $role)
                            @if(isset($edit) && count($edit->roles) > 0)
                            @foreach($edit->roles AS $user_role)
                            <option value="{{ $role->name }}" @if(isset($user_role)) @if($user_role->id == $role->id) {{ 'selected' }} @endif @else @if(old('role') == $role->id) {{ 'selected' }} @endif @endif> {{ ucfirst($role->name) }}</option>
                            @endforeach
                            @else
                            <option value="{{ $role->name }}" @if(isset($user_role)) @if($user_role->id == $role->id) {{ 'selected' }} @endif @else @if(old('role') == $role->id) {{ 'selected' }} @endif @endif> {{ ucfirst($role->name) }}</option>
                            @endif
